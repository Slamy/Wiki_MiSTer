...
* **buttons**: This indicates the default button mapping for the game. As the name implies, it only implies to buttons, not to directional controls. The **names**-attribute specifies how the buttons are called in the MiSTer OSD while the **default**-attribute specifies how the buttons are mapped to the virtual [MiSTer gamepad](#mister-gamepad).
