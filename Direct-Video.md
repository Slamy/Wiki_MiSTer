...
The resulting analog video signal from your DAC should be compatible with standard CRTs if the core support standard CRT refresh frequencies (see [analog video compatibility](Analog-video-output-compatibility) for more details). But there is still a bit of additional configuration you need to do depending on which kind of analog video signal you want to use.
...
- Setup for YPbPr signals: Enable composite sync and YPbPr on your [mister.ini](Configuration-Files) file. Then you'll need to add a sync-on-green circuit to the VGA output. SOG can be very simplified, you just need a diode (1N4148) and a 1k resistor.
